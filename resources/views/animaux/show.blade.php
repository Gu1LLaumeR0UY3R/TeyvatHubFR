...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="mb-6 text-sm text-hub-text-sec">
        <a href="{{ route('animaux.index') }}" class="hover:text-hub-primary">Animaux</a>
        <span class="mx-2">/</span>
        <span class="text-hub-text">{{ $animal->nom_animal }}</span>
    </nav>
    ...
    <div class="bg-hub-surface border border-hub-border rounded-2xl p-6 mb-6">
        <div class="flex flex-col sm:flex-row gap-6">
            ...
            <div class="flex-1">
                <h1 class="text-3xl font-bold text-hub-text mb-2">{{ $animal->nom_animal }}</h1>
                @if($animal->typeAnimal)
    ...
    @if($animal->descri_animal)
        <div class="bg-hub-surface border border-hub-border rounded-2xl p-6 mb-6">
            ...
            <p class="text-hub-text-sec leading-relaxed">{{ $animal->descri_animal }}</p>
        </div>
    @endif
    ...
    {{-- Bloc 4 : Ingrédients --}}
    @if($animal->ingredients->count())
        <div class="bg-hub-surface border border-hub-border rounded-2xl p-6 mb-6">
            <h2 class="text-xl font-bold text-hub-text mb-4">Ingrédients obtenus</h2>
