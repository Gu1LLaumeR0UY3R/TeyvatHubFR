...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-hub-text mb-2">Ennemis</h1>
        <p class="text-hub-text-sec">{{ $ennemis->total() }} ennemi(s) trouvé(s)</p>
    </div>

    <form method="GET" action="{{ route('ennemis.index') }}" class="mb-8 flex flex-wrap gap-3">
        ...
        <select name="type" class="px-4 py-2 bg-hub-surface border border-hub-border rounded-lg text-hub-text focus:outline-none focus:ring-2 focus:ring-hub-primary">
            ...
            @foreach($types as $type)
                <option value="{{ $type->id_typeEnnemi }}" @selected(request('type') == $type->id_typeEnnemi)>
                    {{ $type->libelle_Type }}
                </option>
            @endforeach
        </select>
        ...
        <select name="element" class="px-4 py-2 bg-hub-surface border border-hub-border rounded-lg text-hub-text focus:outline-none focus:ring-2 focus:ring-hub-primary">
            ...
            <option value="neutre" @selected(request('element') === 'neutre')>Neutre</option>
    ...
    @if($ennemis->isEmpty())
        <div class="text-center py-16 text-hub-text-sec">
            <p class="text-lg">Aucun ennemi trouvé.</p>
        </div>
    @else
            ...
            @foreach($ennemis as $ennemi)
                <a href="{{ route('ennemis.show', $ennemi->slug) }}"
                   class="group bg-hub-surface border border-hub-border rounded-xl overflow-hidden hover:border-hub-primary hover:shadow-lg transition-all duration-200">
                    ...
                    <div class="p-3">
                        <p class="font-semibold text-hub-text text-sm truncate">{{ $ennemi->nom_ennemi }}</p>
                        <p class="text-hub-text-sec text-xs mt-1">{{ $ennemi->typeEnnemi?->libelle_Type ?? '—' }}</p>
                        <p class="text-hub-text-sec text-xs">{{ $ennemi->element?->libelle_element ?? 'Neutre' }}</p>
                    </div>
                </a>
            @endforeach
        </div>
