<x-app-layout>
<x-slot name="title">{{ $region->nom_region }}</x-slot>
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="mb-6 text-sm text-hub-text-sec">
        ...
        <a href="{{ route('regions.index') }}" class="hover:text-hub-primary">Régions</a>
        <span class="mx-2">/</span>
        <span class="text-hub-text">{{ $region->nom_region }}</span>
    </nav>
    ...
    <div class="bg-hub-surface border border-hub-border rounded-2xl p-6 mb-6">
        <div class="flex flex-col sm:flex-row gap-6">
            ...
            <div class="flex-1">
                <h1 class="text-3xl font-bold text-hub-text mb-3">{{ $region->nom_region }}</h1>
                @if($region->descri_region)
                    <p class="text-hub-text-sec leading-relaxed">{{ $region->descri_region }}</p>
                @endif
            </div>
        </div>
    </div>
    ...
    {{-- Bloc 4 : Animaux --}}
    @if($region->animaux->count())
        <div class="bg-hub-surface border border-hub-border rounded-2xl p-6 mb-6">
            <h2 class="text-xl font-bold text-hub-text mb-4">Faune</h2>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                @foreach($region->animaux as $animal)
