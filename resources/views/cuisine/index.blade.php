<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h1 class="text-3xl font-bold text-hub-primary mb-6">Cuisine</h1>

        <form method="GET" action="{{ route('cuisine.index') }}" class="mb-6 flex flex-wrap gap-3 items-end">
            ...
            <select name="rarete" class="rounded-lg bg-hub-surface border border-hub-border px-3 py-2 text-hub-text focus:outline-none focus:ring-2 focus:ring-hub-primary">
                <option value="">Toutes les raretés</option>
                @foreach($raretés as $rarete)
                    <option value="{{ $rarete->{'id_rareté'} }}" {{ request('rarete') == $rarete->{'id_rareté'} ? 'selected' : '' }}>
                        {{ $rarete->{'libelle_rareté'} }}
                    </option>
                @endforeach
            </select>
        ...
        @if($plats->isEmpty())
            <div class="text-center py-16 text-hub-text-sec">
                <p class="text-xl">Aucun plat trouvé.</p>
            </div>
        @else
                        ...
                        <a href="{{ route('cuisine.show', $plat->slug) }}"
                           class="flex items-center gap-4 bg-hub-surface border border-hub-border rounded-xl p-3 hover:border-hub-primary transition-colors">
                            <img src="{{ $plat->photos->first()?->source_url ?? $plat->photos->first()?->chemin_photo ?? asset('images/placeholder.webp') }}"
                                 alt="{{ $plat->nom_plat }}" class="w-12 h-12 rounded-full object-cover">
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-hub-text truncate">{{ $plat->nom_plat }}</p>
                                <p class="text-sm text-hub-gold">{{ $plat->rarete?->{'libelle_rareté'} ?? '—' }}</p>
                            </div>
                        </a>
                    @endforeach
                </div>
                        ...
                        <a href="{{ route('cuisine.show', $plat->slug) }}"
                           class="bg-hub-surface border border-hub-border rounded-2xl overflow-hidden hover:border-hub-primary hover:-translate-y-1 transition-all">
                            ...
                            <div class="p-3">
                                ...
                                <p class="text-xs text-hub-gold">{{ $plat->rarete?->{'libelle_rareté'} ?? '—' }}</p>
                            </div>
                        </a>
                    @endforeach
                </div>
