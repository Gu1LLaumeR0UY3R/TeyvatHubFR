<x-admin-layout>
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.nations.create') }}" class="px-4 py-2 bg-hub-gold text-hub-bg rounded hover:opacity-90">+ Nouveau</a>
    </div>
    ...
    <form method="GET" action="{{ route('admin.nations.index') }}" class="mb-4 flex items-end gap-3">
        <div>
    ...
    <div x-data="{
        ...
        <template x-if="selectedCount > 0">
            <div class="p-4 bg-hub-gold/10 border border-hub-gold rounded-lg">
                <h3 class="text-hub-text font-semibold mb-3">Modification en masse (<span x-text="selectedCount"></span> nation(s) sélectionnée(s))</h3>
                <form action="{{ route('admin.nations.bulk-update') }}" method="POST" class="flex flex-wrap gap-3 items-end">
                    @csrf @method('PATCH')
                    ...
                    <div class="min-w-64">
                        <label class="block text-hub-text-sec text-xs mb-1">Description (appliquée à toutes)</label>
                        <input type="text" name="descri_region" class="w-full bg-hub-surface border border-hub-border rounded px-3 py-2 text-hub-text text-sm" placeholder="Nouvelle description">
                    </div>
                    ...
                    <button type="submit" name="action" value="delete" formnovalidate
                            ...
                            onclick="return confirm('Supprimer les nations sélectionnées ?')">
        ...
        <div class="bg-hub-surface rounded-lg overflow-hidden">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-hub-border">
                    @forelse($nations as $nation)
                        <tr class="hub-surface-hover">
                            <td class="px-3 py-2 text-center">
                                <input type="checkbox" x-model="selected['{{ $nation->id_region }}']" class="rounded">
                            </td>
                            <td class="px-3 py-2 text-center">
                                <img src="{{ $nation->icone_url }}" alt="{{ $nation->nom_region }}" class="w-10 h-10 object-contain rounded-full bg-hub-surface-hover mx-auto">
                            </td>
                            <td class="px-4 py-3">{{ $nation->nom_region }}</td>
                            <td class="px-4 py-3 flex gap-2">
                                <a href="{{ route('admin.nations.edit', $nation) }}" class="text-hub-gold hover:underline">Modifier</a>
                                <form action="{{ route('admin.nations.destroy', $nation) }}" method="POST" onsubmit="return confirm('Supprimer ?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="4" class="px-4 py-6 text-center text-hub-text-sec">Aucune région</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
