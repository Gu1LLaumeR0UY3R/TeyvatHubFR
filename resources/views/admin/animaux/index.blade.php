<x-admin-layout>
    ...
    <form method="GET" action="{{ route('admin.animaux.index') }}" class="mb-4 flex items-end gap-3">
        <div>
    ...
    <div x-data="{
        ...
        <template x-if="selectedCount > 0">
            <div class="p-4 bg-hub-gold/10 border border-hub-gold rounded-lg">
                <h3 class="text-hub-text font-semibold mb-3">Modification en masse (<span x-text="selectedCount"></span> animal(aux) sélectionné(s))</h3>
                <form action="{{ route('admin.animaux.bulk-update') }}" method="POST" class="flex flex-wrap gap-3 items-end">
                    @csrf @method('PATCH')
                    ...
                    <div>
                        ...
                        <select name="fid_TAnimal" class="bg-hub-surface border border-hub-border rounded px-3 py-2 text-hub-text text-sm">
                            <option value="">(ne pas changer)</option>
                            @foreach($typesAnimal as $type)
                                <option value="{{ $type->id_TAnimal }}">{{ $type->libelle_TAnimal }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <button type="submit" name="action" value="delete" formnovalidate
                            ...
                            onclick="return confirm('Supprimer les animaux sélectionnés ?')">
        ...
        <div class="bg-hub-surface rounded-lg overflow-hidden">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-hub-border">
                    @forelse($animaux as $animal)
                        <tr class="hub-surface-hover">
                            <td class="px-3 py-2 text-center">
                                <input type="checkbox" x-model="selected['{{ $animal->id_animal }}']" class="rounded">
                            </td>
                            <td class="px-3 py-2 text-center">
                                <img src="{{ $animal->icone_url }}" alt="{{ $animal->nom_animal }}" class="w-10 h-10 object-contain rounded-full bg-hub-surface-hover mx-auto">
                            </td>
                    ...
                    @empty
                        <tr><td colspan="5" class="px-4 py-6 text-center text-hub-text-sec">Aucun animal</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
