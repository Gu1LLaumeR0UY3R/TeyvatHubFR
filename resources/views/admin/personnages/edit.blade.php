<x-admin-layout>
    ...
    <h1 class="text-2xl font-bold text-hub-gold mb-6">Modifier : {{ $personnage->nom_perso }}</h1>

    <form action="{{ route('admin.personnages.update', $personnage) }}" method="POST" enctype="multipart/form-data" class="max-w-lg space-y-4">
        @csrf @method('PUT')

        <div>
            <label class="block text-hub-text-sec mb-1">Nom</label>
            <input type="text" name="nom_perso" value="{{ old('nom_perso', $personnage->nom_perso) }}" required
        ...
        <div>
            ...
            <select name="fid_element" class="w-full bg-hub-surface border border-hub-border rounded px-3 py-2 text-hub-text">
                ...
                @foreach($elements as $el)
                    <option value="{{ $el->id_element }}" {{ old('fid_element', $personnage->fid_element) == $el->id_element ? 'selected' : '' }}>{{ $el->libelle_element }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="fid_etoile" required class="w-full bg-hub-surface border border-hub-border rounded px-3 py-2 text-hub-text">
                @foreach($etoiles as $etoile)
                    <option value="{{ $etoile->id_etoile }}" {{ old('fid_etoile', $personnage->fid_etoile) == $etoile->id_etoile ? 'selected' : '' }}>{{ $etoile->libelle }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="fid_TArmes" class="w-full bg-hub-surface border border-hub-border rounded px-3 py-2 text-hub-text">
                ...
                @foreach($typesArme as $ta)
                    <option value="{{ $ta->id_TArmes }}" {{ old('fid_TArmes', $personnage->fid_TArmes) == $ta->id_TArmes ? 'selected' : '' }}>{{ $ta->libelle_TArme }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="fid_TP" required class="w-full bg-hub-surface border border-hub-border rounded px-3 py-2 text-hub-text">
                @foreach($typesPerso as $tp)
                    <option value="{{ $tp->id_TP }}" {{ old('fid_TP', $personnage->fid_TP) == $tp->id_TP ? 'selected' : '' }}>{{ $tp->libelle_TP }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            @if($personnage->photos->first())
                <img src="{{ $personnage->photos->first()->source_url ?? asset('storage/'.$personnage->photos->first()->chemin_photo) }}"
                     alt="{{ $personnage->nom_perso }}" class="w-16 h-16 object-contain mb-2">
            @endif
