...
<x-admin-layout>
    <x-slot name="title">Nouvel admin</x-slot>
    <h1 class="text-2xl font-bold text-hub-gold mb-6">Creer un admin</h1>
    ...
    <form method="POST" action="{{ route('admin.admins.store') }}" class="max-w-lg space-y-5" enctype="multipart/form-data">
        @csrf

        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Pseudo</label>
            <input name="pseudo_admin" type="text" required value="{{ old('pseudo_admin') }}" class="w-full rounded border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-hub-gold">
        </div>

        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Email</label>
            <input name="email_admin" type="email" required value="{{ old('email_admin') }}" class="w-full rounded border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-hub-gold">
        </div>
        ...
        <div x-data="PhotoCropper()" class="space-y-4">
            ...
            <div @dragover.prevent="dragActive = true" @dragleave="dragActive = false" @drop.prevent="dragActive = false; handleDrop($event)" :class="dragActive ? 'bg-hub-gold/10 border-hub-gold' : 'bg-slate-50 border-slate-300'" class="relative w-full rounded border-2 border-dashed px-6 py-8 transition-colors cursor-pointer text-center">
                ...
                <div x-show="!finalImage" class="text-center pointer-events-none">
                    <p class="mt-2 text-sm text-slate-600">Deposez votre image ici ou cliquez</p>
                </div>
                <div x-show="finalImage" class="pointer-events-none">
                    ...
                    <button type="button" @click="reset()" class="mt-2 text-xs text-slate-500 hover:text-slate-700 pointer-events-auto">Changer</button>
                </div>
            </div>
            ...
            <div>
                ...
                <input name="photo_profil_url" type="url" value="{{ old('photo_profil_url') }}" placeholder="https://..." class="w-full rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-hub-gold">
            </div>
        </div>
        ...
        <div x-data="BannerCropper()" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-medium text-slate-500 mb-1">Ou coller un lien d'image</label>
                <input name="banniere_admin_url" type="url" value="{{ old('banniere_admin_url') }}" placeholder="https://..." class="w-full rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-hub-gold">
            </div>
        </div>
        ...
        <div x-data="{ showPassword: false }" class="space-y-4">
            <div>
                ...
                <div class="flex gap-2">
                    <input :type="showPassword ? 'text' : 'password'" name="mot_de_passe_admin" required autocomplete="new-password" class="flex-1 rounded border border-slate-300 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-hub-gold">
            ...
            <div>
                ...
                <div class="flex gap-2" x-data="{ show: false }">
                    <input :type="show ? 'text' : 'password'" name="mot_de_passe_admin_confirmation" required autocomplete="new-password" class="flex-1 rounded border border-slate-300 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-hub-gold">
        ...
        <div x-data="{ isSuperAdmin: {{ old('role', 'moderateur') === 'super_admin' ? 'true' : 'false' }} }" class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Role</label>
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="role" value="super_admin" {{ old('role') === 'super_admin' ? 'checked' : '' }} @change="isSuperAdmin = true" class="accent-hub-gold">
                        <span class="text-sm text-slate-700">Super Admin</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="role" value="moderateur" {{ old('role', 'moderateur') === 'moderateur' ? 'checked' : '' }} @change="isSuperAdmin = false" class="accent-hub-gold">
                ...
                <div class="space-y-3">
                    @foreach($allPermissions as $perm)
                        <label class="flex items-start gap-3 cursor-pointer">
                            <input type="checkbox" name="permissions[]" value="{{ $perm }}" {{ in_array($perm, (array) old('permissions', [])) ? 'checked' : '' }} class="mt-0.5 h-4 w-4 accent-hub-gold">
                            <span>
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="px-4 py-2 bg-hub-gold text-hub-bg rounded hover:opacity-90">Creer</button>
